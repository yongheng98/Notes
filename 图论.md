...
**拓扑排序** 是指对一个有向无环图的节点进行排序之后得到的序列。如果存在一条从节点A指向节点B的边，那么在拓扑排序的序列节点A出现在节点B的前面。**一个有向无环图可以有一个或者多个拓扑排序的序列，但是无向图或有环的有向图都不存在拓扑排序的序列。**

**常用的算法**：入度为0的节点入队，其他所有与它相连的节点的入度减一，并判断他们的入度是否为0，为0入队，否则不操作。



**并查集** 是一种树形的数据结构，用来表示不相交集合的数据。并查集的每个子集都是一个一棵树，每个元素都是树中的一个结点。树中的每个节点有一个指向父节点的指针，树的根节点的指针指向它自己。

​	并查集支持两种操作，即**合并和查找**。 

**合并操作**是将两个子集合并成一个集合，只需要将一个子集对应的数的根节点的指针指向另一个子集对应的树的根节点。

**查找操作**确定某个集合`v` 处于哪个子集中。如果两个节点的根节点相同，那么他们处于同一个集合中。
...
class Solution {
    public boolean isSimilar(String a, String b)
    {
        ...
        // 为什么是相似的
        /*
        题目已经说过了，给的字符串能保证是字母异位词（anagram）；就是说，两个字符的，同种字母的出现次数是相同的，即字符不可能出现 count == 1 的情况。反证法：如果count==1，即，其他位置的字母都相同，只有一个地方的字母不相同，另字母值为X与Y，则X出现的次数在两个字符中必然不相等，则字符不是 字母异位词anagram，与条件冲突，即证count==1不可能发生
        */
    }
    public int findfather(int a, int[] father)
    {
    ...
    public boolean union(int[] father, int x, int y)
    {
        int a = findfather(x, father);
        int b = findfather(y, father);
        if(a != b)
        {
            father[a] = b;
            return true;
    ...
    public int numSimilarGroups(String[] strs) {
        ...
        for(int i = 0; i < strs.length; i++)
        {
            ...
            {
                if(isSimilar(strs[i], strs[j]) && union(father, i, j))
                {
                    ans --;
                }
            }
...
class Solution {
    ...
    public int[] findRedundantConnection(int[][] edges) {
            // 并查集性质，两个不连接子图的点连接不会形成环，一个子图的两个节点连接会形成环
            int n = edges.length;
            int m = edges[0].length;

            int[] father = new int[n + 1]; // 节点i的根节点是father[i]
            for(int i = 0; i < n + 1; i++)
            {
                father[i] = i;
            }
            ...
            for(int i = 0; i < n; i++)
            {
                int a = edges[i][0];
                int b = edges[i][1];
                int root_a = findFather(a, father);
                int root_b = findFather(b, father);
                if(root_a != root_b) // ab的根节点不同，但是有边，就合并ab子图
                {
                    father[root_a] = root_b;
                }
                else // ab根节点相同，并且ab之间有边，则一定形成环
                {
                    ans[0] = a;
                    ans[1] = b;
                }
