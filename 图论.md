...
class Solution {
    public String alienOrder(String[] words) {
        int[][] g = new int[26][26];
        int[] in_dgree = new int[26];
        Arrays.fill(in_dgree, -1);
        for(int i = 0; i < 26; i++)
        {
            for(int j = 0; j < 26; j++) g[i][j] = -1; // 初始化
        }
            ...
            for(int j = i + 1; j < words.length; j++)
            {
                ...
                for(int k = 0; k < len; k++)
                {
                    ...
                    if(a != b)
                    {
                        ...
                        if(in_dgree[b - 'a'] != -1) in_dgree[b - 'a'] ++;
                        else in_dgree[b - 'a'] = 1;
                        if(in_dgree[a - 'a'] == -1) in_dgree[a - 'a'] = 0;
                        break;
                    }
                }
            }
